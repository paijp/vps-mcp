...
Because `jti` is accepted **once**, one token covers exactly one PUT: a workflow that mints a
single token and loops over several files gets 401 from the second file on. The workflow
generated by `deploy_setup` therefore mints a fresh token inside the upload loop, one per
file.
